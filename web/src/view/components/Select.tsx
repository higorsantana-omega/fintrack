            ...
            <RdxSelect.Content className='z-[99] overflow-hidden bg-white rounded-2xl border border-gray-100 shadow-[0px_11px_20px_0px_rgba(0,0,0,0.10)]'>
              ...
              <RdxSelect.Viewport className='p-2'>
                {options.map(op => (
                  <RdxSelect.Item
                    key={op.value}
                    className='p-2 transition-colors text-gray-800 rounded-lg text-sm data-[state=checked]:font-bold data-[highlighted]:bg-gray-50 outline-none'
                    value={op.value}
                  >
                    <RdxSelect.ItemText>{op.label}</RdxSelect.ItemText>
                  </RdxSelect.Item>
                ))}
              </RdxSelect.Viewport>
